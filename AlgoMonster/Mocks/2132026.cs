...
namespace AlgoMonster.Mocks
{
    public static class _2132026
    {
        ...
        public static string Test(string sentence, string[] dictionary)
        {
            // cat
            ...
            foreach (var word in words)
            {
                ...
                {
                    // if first char not present, return right away
                    // else continue

                    if (IsPrensentInDictionary(word[0..i], map))
                    {
                        res.Add(word[0..i]);
                        isPresent = true;
                        break;
        ...
        private static bool IsPrensentInDictionary(string str, HashSet<string> map)
        {
            if (map.Contains(str)) return true;
